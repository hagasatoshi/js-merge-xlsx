    ...
    _createClass(SpreadSheet, [{
        ...
        value: function load(excel, option) {
            ...
            return _bluebird2['default'].props({
                ...
                if (option && option.sheetname) {
                    var target_sheet = _this._sheet_by_name(option.sheetname);
                    if (!target_sheet) {
                        return _bluebird2['default'].reject("sheetname is invalid. Please check if sheet'" + option.sheetname + "' exists in tempalte file");
                    }
                    _this.template_sheet_name = option.sheetname;
                    _this.template_sheet_data = target_sheet.worksheet.sheetData[0].row;
                } else {
        ...
        value: function generate(option) {
            ...
            parseString(this.excel.file('xl/sharedStrings.xml').asText()).then(function (sharedstrings_obj) {
                ...
                _this6.excel.file('xl/sharedStrings.xml', builder.buildObject(sharedstrings_obj));
                //workbook.xml.rels
                _this6.excel.file("xl/_rels/workbook.xml.rels", builder.buildObject(_this6.workbookxml_rels));
                //workbook.xml
        ...
        value: function has_as_shared_string() {
