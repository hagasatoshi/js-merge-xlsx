    ...
    _createClass(SpreadSheet, [{
        ...
        value: function load(excel) {
            ...
            return Promise.props({
                ...
                _this.sharedstrings = templates.sharedstringsObj.sst.si;
                _this.workbookxmlRels = templates.workbookxmlRels;
                _this.workbookxml = templates.workbookxml;
                _this.sheetXmls = templates.sheetXmls;
                _this.sheetXmlsRels = templates.sheetXmlsRels;
                _this.templateSheetName = _this.workbookxml.workbook.sheets[0].sheet[0]['$'].name;
                _this.templateSheetData = _.find(templates.sheetXmls, function (e) {
        ...
        value: function deleteSheet(sheetname) {
            ...
            _.each(this.workbookxmlRels.Relationships.Relationship, function (sheet, index) {
                if (sheet && sheet['$'].Target === targetSheet.path) {
                    _this5.workbookxmlRels.Relationships.Relationship.splice(index, 1);
                }
            });
        ...
        value: function generate(option) {
            ...
            parseString(this.excel.file('xl/sharedStrings.xml').asText()).then(function (sharedstringsObj) {
                ...
                //sheetXmlsRels
                var strTemplateSheetRels = builder.buildObject(_this6.templateSheetRelsData);
                _.each(_this6.sheetXmls, function (sheet) {
                    if (sheet.name) {
                        _this6.excel.file('xl/worksheets/_rels/' + sheet.name + '.rels', strTemplateSheetRels);
                    }
                });
        ...
        value: function _simpleRender(bindData) {
            ...
            return this.excel.file('xl/sharedStrings.xml', Mustache.render(this.excel.file('xl/sharedStrings.xml').asText(), bindData)).generate(option);
        }
        ...
        value: function _sheetByName(sheetname) {
            ...
            var targetFileName = targetFilePath.split('/')[targetFilePath.split('/').length - 1];
            var sheetXml = _.find(this.sheetXmls, function (e) {
                return e.name === targetFileName;
            });
            var sheet = { path: targetFilePath, value: sheetXml };
            return sheet;
        }
        ...
        value: function _sheetRelsByName(sheetname) {
            ...
            var targetName = targetFilePath.split('/')[targetFilePath.split('/').length - 1] + '.rels';
            var sheetXmlRels = _.find(this.sheetXmlsRels, function (e) {
                return e.name === targetName;
            });
            var sheet = { name: targetName, value: sheetXmlRels };
            return sheet;
        }
